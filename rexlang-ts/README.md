...
```rust
use rexlang_ast::expr::intern;
use rexlang_ts::{BuiltinTypeId, Predicate, Type, TypeSystem};

fn main() -> Result<(), rexlang_ts::TypeError> {
    let mut ts = TypeSystem::with_prelude()?;

    // Register an additional function: id :: a -> a
    let a = Type::var(ts.supply.fresh(Some(intern("a"))));
    let scheme = rexlang_ts::generalize(&ts.env, vec![], Type::fun(a.clone(), a));
    ...
    assert!(rexlang_ts::entails(&ts.classes, &[], &field_f32)?);
    Ok(())
}
```
...
The inference entry points are `TypeSystem::infer` (predicates + type) and `TypeSystem::infer_typed` (typed AST + predicates + type), both of which work on `rexlang_ast::expr::Expr`. Literal behavior follows the current language requirement:
