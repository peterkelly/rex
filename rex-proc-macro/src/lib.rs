...
fn expand(ast: &DeriveInput) -> Result<TokenStream2, Error> {
    ...
    add_bound_to_type_params(&mut rex_type_generics, parse_quote!(::rex_core::RexType));
    ...
    let rex_type_params = type_param_idents.iter().map(|ident| {
        quote! { <#ident as ::rex_core::RexType>::rex_type() }
    });
    let rex_type_impl = quote! {
        impl #rex_type_impl_generics ::rex_core::RexType for #rust_ident #rex_type_ty_generics #rex_type_where_clause {
            fn rex_type() -> ::rex_core::Type {
                let mut ty = ::rex_core::Type::con(#type_name, #type_param_count);
                #( ty = ::rex_core::Type::app(ty, #rex_type_params); )*
                ty
            }
        }
    };
    ...
    add_bound_to_type_params(&mut rex_adt_generics, parse_quote!(::rex_core::RexType));
    let (rex_adt_impl_generics, rex_adt_ty_generics, rex_adt_where_clause) =
        rex_adt_generics.split_for_impl();
    let rex_adt_impl = quote! {
        impl #rex_adt_impl_generics ::rex_core::RexAdt for #rust_ident #rex_adt_ty_generics #rex_adt_where_clause {
            fn rex_adt_decl<State: Clone + Send + Sync + 'static>(
                engine: &mut ::rex_core::Engine<State>,
            ) -> Result<::rex_core::AdtDecl, ::rex_core::EngineError> {
                #adt_decl_fn
            }
        }
    };
    let inject_fn = quote! {
        impl #rex_adt_impl_generics #rust_ident #rex_adt_ty_generics #rex_adt_where_clause {
            pub fn inject_rex<State: Clone + Send + Sync + 'static>(
                engine: &mut ::rex_core::Engine<State>,
            ) -> Result<(), ::rex_core::EngineError> {
                <Self as ::rex_core::RexAdt>::inject_rex(engine)
            }

            pub fn rex_adt_decl<State: Clone + Send + Sync + 'static>(
                engine: &mut ::rex_core::Engine<State>,
            ) -> Result<::rex_core::AdtDecl, ::rex_core::EngineError> {
                <Self as ::rex_core::RexAdt>::rex_adt_decl(engine)
            }
            ...
            where
                Self: ::rex_core::RexDefault<State>,
            {
                <Self as ::rex_core::RexAdt>::inject_rex(engine)?;
...
fn adt_decl_fn(
    ...
    let adt_decl = if param_names.is_empty() {
        quote! {
            let head = ::rex_core::Type::con(#type_name, 0);
            let mut adt = engine.adt_decl_from_type_with_params(&head, &[])?;
        }
    } else {
        quote! {
            let head = ::rex_core::Type::con(#type_name, #param_count);
    ...
    for p in type_params {
        ...
        param_bindings.push(quote! {
            let #p_ident = adt
                .param_type(&::rex_core::intern(#p_lit))
                .ok_or_else(|| ::rex_core::EngineError::UnknownType(::rex_core::intern(#type_name)))?;
        });
        ...
        Data::Struct(data) => match &data.fields {
            Fields::Named(fields) => {
                ...
                for field in &fields.named {
                    ...
                    field_inits.push(quote! {
                        ( ::rex_core::intern(#field_name), #field_ty )
                    });
                }
                Ok(quote! {{
                    #adt_decl
                    #(#param_bindings)*
                    let record = ::rex_core::Type::record(::std::vec![#(#field_inits,)*]);
                    adt.add_variant(::rex_core::intern(#ctor), ::std::vec![record]);
                    Ok(adt)
                }})
            }
            Fields::Unnamed(fields) => {
                ...
                Ok(quote! {{
                    #adt_decl
                    #(#param_bindings)*
                    adt.add_variant(::rex_core::intern(#ctor), ::std::vec![#(#args,)*]);
                    Ok(adt)
                }})
            }
            Fields::Unit => Ok(quote! {{
                #adt_decl
                #(#param_bindings)*
                adt.add_variant(::rex_core::intern(#type_name), ::std::vec![]);
                Ok(adt)
            }}),
        },
        Data::Enum(data) => {
            ...
            for variant in &data.variants {
                ...
                let args = match &variant.fields {
                    ...
                    Fields::Named(fields) => {
                        ...
                        let record = quote! {
                            ::rex_core::Type::record(::std::vec![#(#field_inits,)*])
                        };
                        vec![record]
                    }
                };
                variants.push(quote! {
                    adt.add_variant(::rex_core::intern(#variant_name), ::std::vec![#(#args,)*]);
                });
            }
            Ok(quote! {{
                #adt_decl
...
fn rex_type_expr(
    ...
    match ty {
        Type::Tuple(tuple) => {
            ...
            Ok(quote! { ::rex_core::Type::tuple(::std::vec![#(#elems,)*]) })
        }
        Type::Path(type_path) => {
            ...
            match ident.as_str() {
                "Vec" => {
                    ...
                    Ok(quote! {
                        ::rex_core::Type::app(
                            ::rex_core::Type::builtin(::rex_core::BuiltinTypeId::List),
                            #inner
                        )
                    })
                }
                "HashMap" | "BTreeMap" => {
                    ...
                    Ok(quote! {
                        ::rex_core::Type::app(
                            ::rex_core::Type::builtin(::rex_core::BuiltinTypeId::Dict),
                            #v
                        )
                    })
                }
                "Option" => {
                    ...
                    Ok(quote! {
                        ::rex_core::Type::app(
                            ::rex_core::Type::builtin(::rex_core::BuiltinTypeId::Option),
                            #inner
                        )
                    })
                }
                "Result" => {
                    ...
                    Ok(quote! {
                        ::rex_core::Type::app(
                            ::rex_core::Type::app(
                                ::rex_core::Type::builtin(::rex_core::BuiltinTypeId::Result),
                                #err
                            ),
                            #ok
                        )
                ...
                _ => Ok(quote! { <#type_path as ::rex_core::RexType>::rex_type() }),
            }
        }
        other => Err(Error::new(
            other.span(),
        ...
        Type::Path(type_path) => {
            ...
            match ident.as_str() {
                "Vec" => {
                    ...
                    Ok(quote! {{
                        let mut out =
                            heap.alloc_adt(::rex_core::intern("Empty"), ::std::vec::Vec::new())?;
                        for item in #expr.into_iter().rev() {
                            out = heap
                                .alloc_adt(
                                    ::rex_core::intern("Cons"),
                ...
                "HashMap" | "BTreeMap" => {
                    ...
                    Ok(quote! {{
                        ...
                        for (k, v) in #expr {
                            out.insert(::rex_core::intern(&k), #v_encode);
                        }
                        heap.alloc_dict(out)?
                    }})
                }
                "Option" => {
                    ...
                    Ok(quote! {{
                        match #expr {
                            Some(v) => heap
                                .alloc_adt(::rex_core::intern("Some"), ::std::vec![#inner_encode])?,
                            None => heap
                                .alloc_adt(::rex_core::intern("None"), ::std::vec::Vec::new())?,
                        }
                    }})
                }
                "Result" => {
                    ...
                    Ok(quote! {{
                        match #expr {
                            Ok(v) => heap
                                .alloc_adt(::rex_core::intern("Ok"), ::std::vec![#ok_encode])?,
                            Err(e) => heap
                                .alloc_adt(::rex_core::intern("Err"), ::std::vec![#err_encode])?,
                        }
                    }})
                }
                _ => Ok(quote! { ::rex_core::IntoPointer::into_pointer(#expr, heap)? }),
            }
        }
        other => Err(Error::new(
            other.span(),
...
fn from_value_expr(
    ...
    match ty {
        Type::Tuple(tuple) => {
            ...
            Ok(quote! {{
                match #value_expr {
                    ::rex_core::Value::Tuple(items) if items.len() == #len => {
                        Ok((#(#decs?,)*))
                    }
                    other => Err(::rex_core::EngineError::NativeType { expected: "tuple".into(),
                        got: ::rex_core::value_debug(heap, &other)
            ...
            match ident.as_str() {
                "Vec" => {
                    ...
                    Ok(quote! {{
                        ...
                        loop {
                            match &cur {
                                ::rex_core::Value::Adt(tag, args) if tag.as_ref() == "Empty" && args.is_empty() => {
                                    break Ok(out);
                                }
                                ::rex_core::Value::Adt(tag, args) if tag.as_ref() == "Cons" && args.len() == 2 => {
                                    let v = #inner_decode?;
                                    out.push(v);
                                    cur = heap.get(&args[1])?.as_ref().clone();
                                }
                                other => {
                                    break Err(::rex_core::EngineError::NativeType { expected: "list".into(),
                                        got: ::rex_core::value_debug(heap, &other)
                                            .unwrap_or_else(|err| format!("<display error: {err}>")),
                                    });
                                }
                ...
                "HashMap" | "BTreeMap" => {
                    ...
                    Ok(quote! {{
                        match #value_expr {
                            ...
                            other => Err(::rex_core::EngineError::NativeType { expected: "dict".into(),
                                got: ::rex_core::value_debug(heap, &other)
                                    .unwrap_or_else(|err| format!("<display error: {err}>")),
                            }),
                        }
                ...
                "Option" => {
                    ...
                    Ok(quote! {{
                        match #value_expr {
                            ::rex_core::Value::Adt(tag, args) if tag.as_ref() == "None" && args.is_empty() => Ok(None),
                            ::rex_core::Value::Adt(tag, args) if tag.as_ref() == "Some" && args.len() == 1 => Ok(Some(#inner_decode?)),
                            other => Err(::rex_core::EngineError::NativeType { expected: "option".into(),
                                got: ::rex_core::value_debug(heap, &other)
                                    .unwrap_or_else(|err| format!("<display error: {err}>")),
                            }),
                        }
                ...
                "Result" => {
                    ...
                    Ok(quote! {{
                        match #value_expr {
                            ::rex_core::Value::Adt(tag, args) if tag.as_ref() == "Ok" && args.len() == 1 => Ok(Ok(#ok_decode?)),
                            ::rex_core::Value::Adt(tag, args) if tag.as_ref() == "Err" && args.len() == 1 => Ok(Err(#err_decode?)),
                            other => Err(::rex_core::EngineError::NativeType { expected: "result".into(),
                                got: ::rex_core::value_debug(heap, &other)
                                    .unwrap_or_else(|err| format!("<display error: {err}>")),
                            }),
                        }
                ...
                _ => Ok(quote! {{
                    let __rex_value: ::rex_core::Value = (#value_expr).clone();
                    let __rex_ptr = heap.alloc_value(__rex_value)?;
                    <#type_path as ::rex_core::FromPointer>::from_pointer(heap, &__rex_ptr)
                }}),
            }
        }
        other => Err(Error::new(
...
fn into_value_impl(ast: &DeriveInput, type_name: &str) -> Result<TokenStream2, Error> {
        ...
        Data::Struct(data) => match &data.fields {
            Fields::Named(fields) => {
                ...
                for field in &fields.named {
                    ...
                    inserts.push(quote! {
                        map.insert(::rex_core::intern(#name), #enc);
                    });
                }
                quote! {{
                    ...
                    heap.alloc_adt(::rex_core::intern(#ctor), ::std::vec![dict])?
                }}
            }
            Fields::Unnamed(fields) => {
                let mut args = Vec::new();
                ...
                quote! {{
                    ...
                    heap.alloc_adt(::rex_core::intern(#ctor), ::std::vec![#(#args,)*])?
                }}
            }
            Fields::Unit => quote! {
                heap.alloc_adt(::rex_core::intern(#ctor), ::std::vec::Vec::new())?
            },
        },
        Data::Enum(data) => {
                ...
                let arm = match &variant.fields {
                    Fields::Unit => quote! {
                        Self::#variant_ident => heap
                            .alloc_adt(::rex_core::intern(#variant_name), ::std::vec::Vec::new())?
                    },
                    Fields::Unnamed(fields) => {
                        ...
                        quote! {
                            Self::#variant_ident(#(#vars,)*) => heap
                                .alloc_adt(::rex_core::intern(#variant_name), ::std::vec![#(#encs,)*])?
                        }
                    }
                    Fields::Named(fields) => {
                        ...
                        quote! {
                            Self::#variant_ident { #(#vars,)* } => {
                                ...
                                #(#inserts)*
                                let dict = heap.alloc_dict(map)?;
                                heap.alloc_adt(::rex_core::intern(#variant_name), ::std::vec![dict])?
                            }
                        }
                    }
                };
    ...
    add_bound_to_type_params(&mut generics, parse_quote!(::rex_core::IntoPointer));
    let (impl_generics, _, where_clause) = generics.split_for_impl();
    let (_, ty_generics, _) = generics.split_for_impl();

    Ok(quote! {
        impl #impl_generics ::rex_core::IntoPointer for #rust_ident #ty_generics #where_clause {
            fn into_pointer(
                self,
                heap: &::rex_core::Heap,
            ) -> ::std::result::Result<::rex_core::Pointer, ::rex_core::EngineError> {
                Ok(#body)
            }
        }
    })
...
fn from_value_impl(ast: &DeriveInput, type_name: &str) -> Result<TokenStream2, Error> {
        ...
        Data::Struct(data) => match &data.fields {
            Fields::Named(fields) => {
                ...
                for field in &fields.named {
                    ...
                    let key = quote!(::rex_core::intern(#name));
                    ...
                    field_decodes.push(quote! {
                        let v = map.get(&#key).ok_or_else(|| ::rex_core::EngineError::NativeType { expected: format!("missing field `{}`", #name),
                            got: "dict".into(),
                        })?;
                        let #ident = #decode?;
                    });
                ...
                Ok(quote! {{
                    match value {
                        ...
                        other => Err(::rex_core::EngineError::NativeType { expected: #type_name.into(),
                            got: ::rex_core::value_debug(heap, &other)
                                .unwrap_or_else(|err| format!("<display error: {err}>")),
                        }),
                    }
    ...
    let mut generics = ast.generics.clone();
    add_bound_to_type_params(&mut generics, parse_quote!(::rex_core::FromPointer));
    let (impl_generics, _, where_clause) = generics.split_for_impl();
    let (_, ty_generics, _) = generics.split_for_impl();

    Ok(quote! {
        impl #impl_generics ::rex_core::FromPointer for #rust_ident #ty_generics #where_clause {
            fn from_pointer(
                heap: &::rex_core::Heap,
                pointer: &::rex_core::Pointer,
            ) -> Result<Self, ::rex_core::EngineError> {
