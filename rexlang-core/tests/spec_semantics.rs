use rexlang_core::{
    Engine, EngineError, GasMeter, Heap, Parser, Pointer, Token, Type, TypeError, Value,
};
...
async fn spec_defaulting_picks_a_concrete_type_for_numeric_classes() {
    ...
    assert_eq!(ty, Type::builtin(rexlang_core::BuiltinTypeId::F32));
...
async fn spec_integer_literals_unify_with_integral_context() {
    ...
    assert_eq!(ty, Type::builtin(rexlang_core::BuiltinTypeId::U64));
    let value = heap.get(&pointer).unwrap();
    match value.as_ref() {
...
async fn test_let_tuple_destructuring() {
    ...
    let (heap, pointer, ty) = eval("let t = (1, \"Hello\", true), (x, y, z) = t in z")
        .await
        .unwrap();
    assert_eq!(ty, Type::builtin(rexlang_core::BuiltinTypeId::Bool));
    match heap.get(&pointer).unwrap().as_ref() {
