use rexlang_core::{Engine, GasMeter, Heap, Pointer, Type, Value};
...
fn list_elements(heap: &rexlang_core::Heap, list: &Pointer) -> Vec<Pointer> {
    let mut out = Vec::new();
    let mut cur = *list;
    loop {
...
fn list_i32_values(heap: &rexlang_core::Heap, ptr: &Pointer) -> Vec<i32> {
...
async fn demo_fibonacci() {
    ...
    assert_eq!(
        ty,
        Type::list(Type::builtin(rexlang_core::BuiltinTypeId::I32))
    );
    assert_eq!(
