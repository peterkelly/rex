...
fn assert_overload_tuple_type_shape(ty: &Type) {
    ...
    assert_eq!(items[1], Type::builtin(rexlang_core::BuiltinTypeId::String));
    assert_eq!(items[2], Type::builtin(rexlang_core::BuiltinTypeId::Bool));
    assert!(
        is_i32_or_var(&items[3]),
        "expected i32/var at index 3, got {}",
        items[3]
    );
    assert_eq!(items[4], Type::builtin(rexlang_core::BuiltinTypeId::Bool));
    assert_eq!(items[5], Type::builtin(rexlang_core::BuiltinTypeId::String));
}
...
async fn injected_functions_can_read_shared_state_fields() {
    ...
    assert_eq!(
        ty,
        Type::tuple(vec![
            Type::builtin(rexlang_core::BuiltinTypeId::Uuid),
            Type::builtin(rexlang_core::BuiltinTypeId::Uuid),
