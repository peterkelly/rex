...
impl RexAdt for ManualRecord {
    ...
    ) -> Result<rexlang_core::AdtDecl, EngineError> {
...
fn adt_decl_from_type_rejects_non_constructor_heads() {
    ...
    let err = engine
        .adt_decl_from_type(&Type::tuple(vec![Type::builtin(
            rexlang_core::BuiltinTypeId::I32,
        )]))
        .unwrap_err();
...
fn adt_decl_from_type_with_params_validates_arity() {
    let mut engine = Engine::new(());
    let err = engine
        .adt_decl_from_type_with_params(&Type::builtin(rexlang_core::BuiltinTypeId::Result), &["T"])
        .unwrap_err();
    let EngineError::Custom(message) = err else {
        panic!("expected EngineError::Custom");
    };
