...
fn parse_program(source: &str) -> rexlang_core::Program {
...
fn primitive_roundtrip() {
    ...
    let cases = vec![
        (
            Type::builtin(rexlang_core::BuiltinTypeId::Bool),
            json!(true),
        ),
        (Type::builtin(rexlang_core::BuiltinTypeId::I32), json!(-7)),
        (
            Type::builtin(rexlang_core::BuiltinTypeId::String),
            json!("hello"),
        ),
    ];
...
fn option_and_result_roundtrip() {
    ...
    let opt_ty = Type::option(Type::builtin(rexlang_core::BuiltinTypeId::I32));
    let some = json!(9);
    ...
    let res_ty = Type::result(
        Type::builtin(rexlang_core::BuiltinTypeId::I32),
        Type::builtin(rexlang_core::BuiltinTypeId::String),
    );
...
fn json_array_maps_to_array_not_list() {
    ...
    let array_ty = Type::array(Type::builtin(rexlang_core::BuiltinTypeId::I32));
    ...
    let list_ty = Type::list(Type::builtin(rexlang_core::BuiltinTypeId::I32));
...
fn struct_like_single_variant_adt_roundtrip() {
    ...
    foo.add_variant(
        sym("Foo"),
        vec![Type::record(vec![
            (sym("a"), Type::builtin(rexlang_core::BuiltinTypeId::U64)),
            (sym("b"), Type::builtin(rexlang_core::BuiltinTypeId::String)),
        ])],
    );
    ts.inject_adt(&foo);
