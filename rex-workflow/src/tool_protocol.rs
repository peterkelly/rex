//! JSON protocol shared by independently installed Rex tool binaries and rex-workflow.

use crate::{
    modules::tools::executor::{OciToolImages, ToolBundle},
    state::State,
};
use rex::{
...
/// Build the filesystem-CAS/Docker state used by the bundled external tool crates.
pub fn default_tool_state() -> Result<State, Box<dyn Error>> {
    let store_path = std::env::var_os("REX_STORE")
        .map(PathBuf::from)
        .ok_or("REX_STORE must name the shared content-addressed store directory")?;
    let mut images = OciToolImages::current_development();
    for (bundle, variable) in [
        (ToolBundle::Ffmpeg, "REX_WORKFLOW_FFMPEG_IMAGE"),
        (ToolBundle::Gnuplot, "REX_WORKFLOW_GNUPLOT_IMAGE"),
        (ToolBundle::Graphviz, "REX_WORKFLOW_GRAPHVIZ_IMAGE"),
        (ToolBundle::ImageMagick, "REX_WORKFLOW_IMAGEMAGICK_IMAGE"),
        (ToolBundle::Qpdf, "REX_WORKFLOW_QPDF_IMAGE"),
        (ToolBundle::Poppler, "REX_WORKFLOW_POPPLER_IMAGE"),
    ] {
        if let Ok(reference) = std::env::var(variable) {
            images = images.with_image(bundle, reference);
        }
    }
    images.validate()?;
    Ok(State::docker(
        rex::storage::Store::new_with_filesystem(store_path),
        images,
    ))
}
...
impl ToolManifest {
        ...
        M: Fn() -> Result<Module<State>, EngineError> + Copy,
    {
        ...
        let mut builder =
            Builder::with_prelude(State::without_tools(rex::storage::Store::new_in_memory()))?;
...
mod tests {
    ...
    async fn execute_uses_the_typed_json_boundary() {
        let result = execute_module_function(
            test_module().unwrap(),
            State::without_tools(rex::storage::Store::new_in_memory()),
            "increment",
            json!({ "value": 41 }),
        )
        .await
