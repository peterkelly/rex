...
use rex_workflow::modules::tools::executor::{
    ...
    OciJobFuture, OciJobLimits, OciPlatform, OutputKind, ToolArgument, ToolBundle, ToolExecution,
    ...
    fn run<'a>(&'a self, store: &'a Store, job: OciJob) -> OciJobFuture<'a> {
        Box::pin(async move {
            ...
            let fallback_blob = store
                ...
                .await
                .map_err(|error| {
                    rex_workflow::modules::tools::executor::ToolExecutionError::with_kind(
...
fn image(platform: OciPlatform) -> OciImage {
    OciImage {
        ...
        reference: format!("registry.example/rex-ffmpeg@sha256:{DIGEST}"),
        platform,
    }
}
...
async fn remote_boundary_rejects_mutable_images_and_unsupported_policy() {
    ...
    mutable.image.reference = "registry.example/rex-ffmpeg:latest".to_owned();
    assert_eq!(
