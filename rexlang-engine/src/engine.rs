    ...
    State: Clone + Send + Sync + 'static,
{
    ...
    /// use rexlang_ts::{BuiltinTypeId, Type};
    ///
...
///
/// ```rust,ignore
/// use rexlang_engine::collect_adts_error_to_engine;
/// use rexlang_ts::{collect_adts_in_types, Type};
///
    ...
    State: Clone + Send + Sync + 'static,
{
    ...
    fn register_prelude_typeclass_instances(&mut self) -> Result<(), EngineError> {
        ...
        let program = rexlang_ts::prelude_typeclasses_program().map_err(EngineError::Type)?;
...
mod tests {
    ...
    fn parse(code: &str) -> Arc<Expr> {
        let mut parser = rexlang_parser::Parser::new(rexlang_lexer::Token::tokenize(code).unwrap());
    ...
    fn module_add_adt_decls_from_types_collects_nested_unique_adts() {
        let mut engine = Engine::with_prelude(()).unwrap();
        let mut module = Module::new("acme.types");
        let a = Type::var(rexlang_ts::TypeVar::new(0, Some(sym("a"))));
        let types = vec![
            Type::fun(
