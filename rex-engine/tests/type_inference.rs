...
#[test]
fn infer_list_get_type() {
    let expr = parse_expr("list_get (1 is u64) (([1, 2, 3]) is List i32)");
    let mut ts = standard_type_system().unwrap();
    let (preds, ty) = infer(&mut ts, expr.as_ref()).unwrap();
    assert_eq!(ty, Type::option(Type::builtin(BuiltinTypeId::I32)));
    assert!(preds.iter().all(|p| p.class.as_ref() == "Integral"));
    assert!(
        preds.iter().any(|p| {
...
#[test]
fn infer_tuple_projection_type() {
