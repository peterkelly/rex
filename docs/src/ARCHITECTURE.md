...
2. **Parsing** (`rexlang-parser`): converts tokens into a `rexlang_ast::expr::Program { decls, expr }`.
3. **Typing** (`rexlang-ts`): Hindley–Milner inference + ADTs + type classes; produces a `rexlang_ts::TypedExpr`.
...
- `rexlang-ast`: shared AST types (`Expr`, `Pattern`, `Decl`, `TypeExpr`, `Program`, symbols).
...
- `rexlang-ts`: type system. Entry points:
...
- **Typed evaluation**: `rexlang-engine` always evaluates a `TypedExpr`; it typechecks first (via `rexlang-ts`) and then evaluates. This keeps runtime behavior predictable and makes native-function dispatch type-directed.
- **Prelude split**: The type system prelude is a combination of:
  - ADT/typeclass *heads* injected by `TypeSystem::with_prelude()?`
  - typeclass method *bodies* (written in Rex) loaded from `rexlang-ts/src/prelude_typeclasses.rex` and injected by `Engine::with_prelude(state)?` (`state` can be `()`)
