use crate::workflow::executor::{
    OciImage, OciJobExecutor, OciToolExecutor, ToolExecutionError, ToolExecutionErrorKind,
    ToolExecutionPlan, ToolExecutor, ToolFuture, docker_executor,
};
use crate::{modules::std::storage::StateStore, storage::Store};
...
impl State {
    ...
    ) -> Result<crate::workflow::executor::ToolExecution, ToolExecutionError> {
        self.tools.execute(&self.store, plan).await
    }

    pub fn docker(store: Store, image: OciImage, allow_image_tags: bool) -> Self {
        Self {
            store,
            tools: docker_executor(image, allow_image_tags),
            external_tools: None,
        }
    }
    ...
    pub fn oci(
        ...
        Self {
            store,
            tools: Arc::new(
                OciToolExecutor::new(image, backend).with_image_tags_allowed(allow_image_tags),
            ),
            external_tools: None,
        }
    }
