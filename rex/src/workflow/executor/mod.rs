...
/// A trusted command supplied by an installed tool implementation.
#[derive(Clone, Debug, Eq, PartialEq)]
...
impl ToolProgram {
    ...
    pub fn with_prefix_arguments<I, S>(executable: impl Into<String>, arguments: I) -> Self
    where
        I: IntoIterator<Item = S>,
        S: Into<String>,
    {
        let mut command = vec![executable.into()];
...
impl OciPlatform {
    ...
    pub fn native_linux() -> Self {
        Self {
            os: "linux".to_owned(),
            architecture: oci_architecture().to_owned(),
            variant: None,
        }
    }
}
...
/// One host-approved OCI image for an installed tool and target platform.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct OciImage {
    pub name: String,
...
impl OciImage {
    ...
    pub fn validate(&self, allow_tags: bool) -> Result<(), ToolExecutionError> {
        validate_image_reference(&self.name, &self.reference, allow_tags)
    }
}
...
fn validate_image_reference(
            ...
            .any(|character| character.is_whitespace() || character.is_control())
    {
        return Err(ToolExecutionError::with_kind(
            ToolExecutionErrorKind::InvalidJob,
            format!("invalid OCI image reference for {name}"),
        ));
    }
    if !allow_tags && image_digest(reference).is_none() {
        return Err(ToolExecutionError::with_kind(
            ToolExecutionErrorKind::InvalidJob,
            format!("OCI image for {name} must be digest-qualified"),
        ));
    }
    Ok(())
}
...
impl OciToolExecutor {
    pub fn new(image: OciImage, backend: Arc<dyn OciJobExecutor>) -> Self {
        Self {
            image,
            allow_image_tags: false,
            backend,
...
impl ToolExecutor for OciToolExecutor {
    fn execute<'a>(&'a self, store: &'a Store, plan: ToolExecutionPlan) -> ToolFuture<'a> {
        Box::pin(async move {
            self.image.validate(self.allow_image_tags)?;
            let job = OciJob {
                image: self.image.clone(),
                command: plan.program.command,
...
pub fn validate_oci_job(
    ...
    validate_image_reference(&job.image.name, &job.image.reference, true)?;
