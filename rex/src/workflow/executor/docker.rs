use super::{
    OciDigest, OciExecutorCapabilities, OciImage, OciIsolationPolicy, OciJob, OciJobExecutor,
    OciJobFuture, OciJobLimits, OciPlatform, OciToolExecutor, ToolExecution, ToolExecutionError,
...
impl DockerToolExecutor {
    pub fn new(image: OciImage) -> Self {
        let backend = Arc::new(DockerOciJobExecutor::new());
        Self {
            executor: OciToolExecutor::new(image, backend.clone()),
            backend,
        }
    }

    pub fn with_image_tags_allowed(mut self, allow: bool) -> Self {
        self.executor.allow_image_tags = allow;
        self
    }
    ...
    pub fn with_docker_binary(mut self, binary: impl Into<PathBuf>) -> Self {
        ...
        self.executor = OciToolExecutor::new(self.executor.image.clone(), backend.clone())
            .with_image_tags_allowed(self.executor.allow_image_tags)
...
pub fn docker_executor(image: OciImage, allow_image_tags: bool) -> Arc<dyn ToolExecutor> {
    Arc::new(DockerToolExecutor::new(image).with_image_tags_allowed(allow_image_tags))
}
...
async fn execute_docker_job(
    ...
    let Some((executable, prefix_arguments)) = job.command.split_first() else {
        return Err(ToolExecutionError::with_kind(
            ToolExecutionErrorKind::InvalidJob,
            "OCI job command must not be empty",
        ));
    };
...
mod tests {
    use super::*;
    use crate::workflow::executor::{ExpectedOutput, OciImage, OutputKind, ToolProgram};
    ...
    fn image() -> OciImage {
        OciImage::new(
            "example",
            format!("example/runtime@sha256:{DIGEST}"),
            OciPlatform::native_linux(),
        )
    }

    #[test]
    fn mutable_image_references_require_explicit_permission() {
        let image = OciImage::new("example", "example:latest", OciPlatform::native_linux());
    ...
    fn image_references_cannot_be_interpreted_as_docker_options() {
        for image in ["", "--privileged", "image with spaces", "image\n--volume"] {
            assert!(super::super::validate_image_reference("example", image, true).is_err());
        }
    }
    ...
    fn docker_create_is_offline_hardened_and_separates_mounts() {
        ...
        let environment = arguments
            .windows(2)
            .filter_map(|pair| (pair[0] == "--env").then_some(pair[1].as_ref()))
            .collect::<Vec<_>>();
        assert_eq!(
            environment,
            [
    ...
    async fn executor_reads_file_result_and_ignores_transport_io_and_status() {
        ...
        let executor = DockerToolExecutor::new(image()).with_docker_binary(&fake_docker);
        let execution = executor
            .execute(
                &store,
                ToolExecutionPlan {
                    program: ToolProgram::new("example-command"),
                    arguments: Vec::new(),
                    inputs: Vec::new(),
    ...
    async fn executor_timeout_force_removes_the_container() {
        ...
        let executor = DockerToolExecutor::new(image())
