...
fn primitive_roundtrip() {
    ...
    for (ty, expected_json) in cases {
        let handle = json_to_rex(&expected_json, &ty, &ts).unwrap();
...
fn option_and_result_roundtrip() {
    ...
    let some_handle = json_to_rex(&some, &opt_ty, &ts).unwrap();
    let none_handle = json_to_rex(&none, &opt_ty, &ts).unwrap();
    ...
    let ok_handle = json_to_rex(&ok_json, &res_ty, &ts).unwrap();
    let err_handle = json_to_rex(&err_json, &res_ty, &ts).unwrap();
...
fn promise_roundtrip_from_json() {
    ...
    let promise_handle = json_to_rex(&promise_json, &promise_ty, &ts).unwrap();
    let Value::Adt(tag, args) = &promise_handle else {
...
fn promise_roundtrip_from_runtime_value() {
    ...
    let promise_handle = Value::Adt(Symbol::intern("Promise"), vec![Value::Uuid(fixed_uuid())]);

    let promise_json = rex_to_json(&promise_handle, &promise_ty, &ts).unwrap();
    assert_eq!(promise_json, json!(fixed_uuid()));

    let roundtrip_handle = json_to_rex(&promise_json, &promise_ty, &ts).unwrap();
    let Value::Adt(tag, args) = &roundtrip_handle else {
        panic!("expected Promise ADT");
    };
    assert_eq!(tag.as_ref(), "Promise");
    assert_eq!(args.len(), 1);
...
fn json_array_maps_to_list() {
    ...
    let list_handle = json_to_rex(&array_json, &list_ty, &ts).unwrap();
...
fn struct_like_single_variant_adt_roundtrip() {
    ...
    let foo_handle = json_to_rex(&foo_json, &foo_ty, &ts).unwrap();
    let Value::Adt(tag, args) = &foo_handle else {
...
fn unit_enum_string_roundtrip() {
    ...
    for v in [json!("Red"), json!("Green"), json!("Blue")] {
        let handle = json_to_rex(&v, &color_ty, &ts).unwrap();
...
fn rex_to_json_rejects_qualified_foreign_constructor() {
    ...
    assert!(rex_to_json(&value, &color_ty, &ts).is_err());
}
