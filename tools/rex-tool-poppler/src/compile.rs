...
pub(crate) fn pdfinfo_plan(
    ...
    let mut builder = PlanBuilder::new(ToolProgram::new("pdfinfo"));
...
pub(crate) fn pdftocairo_plan(
    ...
    let mut builder = PlanBuilder::new(ToolProgram::new("pdftocairo"));
    let input = builder.input_pdf(pdf);
...
pub(crate) fn pdfimages_plan(
    ...
    validate_page_range(options.first_page, options.last_page)?;
    let mut builder = PlanBuilder::new(ToolProgram::new("pdfimages"));
    let input = builder.input_pdf(pdf);
...
mod tests {
    ...
    fn pdftotext_uses_symbolic_paths_and_documented_flags() {
        ...
        .unwrap();
        assert_eq!(plan.program, ToolProgram::new("pdftotext"));
        assert!(
