...
pub const IMAGE_ENVIRONMENT_VARIABLE: &str = "REX_WORKFLOW_GRAPHVIZ_IMAGE";
pub const DEFAULT_IMAGE: &str = "rex-tool-graphviz:local";
...
mod examples {
    ...
    async fn typecheck() {
        rex::workflow::tool_protocol::typecheck_tool_examples(
            crate::tool::module,
            &std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../examples/graphviz"),
        )
        .await
        .unwrap();
    }
