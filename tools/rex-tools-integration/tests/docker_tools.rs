...
use rex::workflow::{
    executor::{
        CasInput, DockerToolExecutor, ExpectedOutput, InputKind, OciImage, OciPlatform, OutputKind,
...
fn docker_fixture() -> Option<DockerFixture> {
    ...
    let tool_directory = std::env::current_exe()
        ...
        .to_path_buf();
    let mut state = State::without_tools(store.clone())
...
fn docker_executor_fixture(
    name: &str,
    image_environment: &str,
    default_image: &str,
) -> Option<(Store, DockerToolExecutor)> {
    if !docker_tests_enabled() {
        eprintln!("skipping Docker integration test; set {ENABLE_ENV}=1 to enable the suite");
        return None;
    }
    let image = OciImage::new(
        name,
        image_reference(image_environment, default_image),
        OciPlatform::native_linux(),
    );
    Some((
        Store::new_in_memory(),
        DockerToolExecutor::new(image).with_image_tags_allowed(true),
    ))
}
...
async fn docker_supports_every_output_kind() {
    let Some((store, executor)) =
        docker_executor_fixture("ffmpeg", FFMPEG_IMAGE_ENV, "rex-tool-ffmpeg:local")
    else {
        return;
    };
    for (kind, color) in [
...
async fn docker_enforces_input_root_host_and_network_isolation() {
    ...
    let hidden_host_file = executor
        .execute(
            &store,
            literal_plan(
                ToolProgram::new("ffprobe"),
                &["-v", "error", &sentinel_path],
            ),
        )
        .await
        .unwrap();
...
async fn docker_uses_packaged_and_cas_supplied_fonts() {
    let Some((store, executor)) = docker_executor_fixture(
        "imagemagick",
        IMAGEMAGICK_IMAGE_ENV,
    ...
    let fallback = ToolExecutionPlan {
        program: ToolProgram::new("magick"),
        arguments: [
            "-background",
            "white",
            "-fill",
...
async fn docker_distinguishes_tool_failures_from_missing_images() {
    ...
    let failure = executor
        .execute(
            ...
            literal_plan(
                ToolProgram::new("ffmpeg"),
                &["--definitely-not-an-ffmpeg-option"],
            ),
        )
        .await
    ...
    let missing = DockerToolExecutor::new(OciImage::new(
        "missing",
        "rex-tool-image-that-does-not-exist:missing",
        OciPlatform::native_linux(),
    ))
    .with_image_tags_allowed(true);
    let error = missing
        .execute(
            &store,
            literal_plan(ToolProgram::new("ffmpeg"), &["-version"]),
        )
        .await
